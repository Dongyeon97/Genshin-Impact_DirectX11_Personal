...
void CRect_RenderTarget::Initialize()
{
...
_int CRect_RenderTarget::PreUpdate(const _float& _fTimeDelta)
{
	return CPrimitiveComponent::PreUpdate(_fTimeDelta);
}

_int CRect_RenderTarget::Update(const _float& _fTimeDelta)
{
	return CPrimitiveComponent::Update(_fTimeDelta);
}

_int CRect_RenderTarget::LateUpdate(const _float& _fTimeDelta)
{
	return CPrimitiveComponent::LateUpdate(_fTimeDelta);
}

void CRect_RenderTarget::Render()
{
	CPrimitiveComponent::Render();
}

void CRect_RenderTarget::EndPlay()
{
	CPrimitiveComponent::EndPlay();
}

void CRect_RenderTarget::Release()
{
	if (m_pEffect)
		m_pEffect->Release();
...
CRect_RenderTarget* CRect_RenderTarget::Create()
{
	CRect_RenderTarget* pInstance = new CRect_RenderTarget();

	pInstance->SetShaderFileInfo(L"Shader_Deferred");
	pInstance->SetVertexInfo(VTXPOSTEX::Elements, VTXPOSTEX::iNumElements);
...
HRESULT CRect_RenderTarget::BindShaderResource(const _char* _pConstantName, ID3D11ShaderResourceView* pSRV)
{
...
HRESULT CRect_RenderTarget::BeginShader(_uint _iPassIndex, _uint _iTechniqueIndex)
{
...
HRESULT CRect_RenderTarget::BindRawValue(const _char* _pConstantName, const void* _pData, _uint _iLength)
{
	ID3DX11EffectVariable* pVariable = m_pEffect->GetVariableByName(_pConstantName);
	if (nullptr == pVariable)
		return E_FAIL;
...
HRESULT CRect_RenderTarget::BindMatrix(const _char* _pConstantName, const _float4x4* _pMatrix)
{
	ID3DX11EffectVariable* pVariable = m_pEffect->GetVariableByName(_pConstantName);
	if (nullptr == pVariable)
		return E_FAIL;
...
HRESULT CRect_RenderTarget::BindMatrices(const _char* _pConstantName, const _float4x4* _pMatrices, _uint _iNumMatrices)
{
	ID3DX11EffectVariable* pVariable = m_pEffect->GetVariableByName(_pConstantName);
	if (nullptr == pVariable)
		return E_FAIL;
...
HRESULT CRect_RenderTarget::InitializeBuffer()
{
...
HRESULT CRect_RenderTarget::CreateShader(const wstring& _wstrShaderFileName, const D3D11_INPUT_ELEMENT_DESC* _pElements, _uint _iNumElements)
{
	_uint		iHlslFlag = { 0 };
...
void CRect_RenderTarget::SetShaderFileInfo(const wstring& _wstrShaderFileName)
{
	m_wstrShaderFileName = _wstrShaderFileName;
}

void CRect_RenderTarget::SetVertexInfo(const D3D11_INPUT_ELEMENT_DESC* _pElements, _uint _iNumElements)
{
