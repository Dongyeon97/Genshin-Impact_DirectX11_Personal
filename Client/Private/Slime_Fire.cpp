...
void CSlime_Fire::Initialize()
{
	// 프리팹 매니저에서 모델 프리팹 검색, 있으면 데이터를 받아옴
	// m_ModelData = *CPrefabManager::GetInstance()->GetModelData(L"Monster_Slime_Fire_02 (merge)");
	// m_SpawnData = CSpawnManager::GetInstance()->FindSpawnData("Slime_Fire");

	m_wpStateMachine = CreateActorComponent<CSlime_Fire_StateMachine>(L"Slime_Fire_StateMachine");
...
#pragma endregion 몬스터 정보
	...
	m_wpAnimModel.lock()->GetAnimVector()[SLIME_FIRE_ANIM::WALK_FORWARD]->SetIsLoop(true);
	m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::IDLE);

	m_wpStateMachine.lock()->SetAnimModel(m_wpAnimModel.lock());
	dynamic_pointer_cast<CSlime_Fire_StateMachine>(m_wpStateMachine.lock())->AddAttackCollider(L"AttackCollider", m_spAttackCollider);
}

_int CSlime_Fire::PreUpdate(const _float& _fTimeDelta)
{
	return CNormalMonster::PreUpdate(_fTimeDelta);
}

_int CSlime_Fire::Update(const _float& _fTimeDelta)
{
	return CNormalMonster::Update(_fTimeDelta);
}

_int CSlime_Fire::LateUpdate(const _float& _fTimeDelta)
{
...
void CSlime_Fire::Render()
{
	CNormalMonster::Render();
}

void CSlime_Fire::EndPlay()
{
	CNormalMonster::EndPlay();
}

void CSlime_Fire::Release()
{
