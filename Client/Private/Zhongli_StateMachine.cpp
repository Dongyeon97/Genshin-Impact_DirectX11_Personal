...
void CZhongli_StateMachine::Initialize()
{
	...
	m_Attack1_Desc	=	{ wstrOwnerName, L"Player_Zhongli Attack1", NONE_ELEMENTAL, WEAK,		false,	fAtk * 1.0f };
	m_Attack2_Desc	=	{ wstrOwnerName, L"Player_Zhongli Attack2", NONE_ELEMENTAL, WEAK,		false,	fAtk * 1.2f };
	m_Attack3_Desc	=	{ wstrOwnerName, L"Player_Zhongli Attack3", NONE_ELEMENTAL, WEAK,		false,	fAtk * 1.4f };
	m_Attack4_Desc	=	{ wstrOwnerName, L"Player_Zhongli Attack4", NONE_ELEMENTAL, WEAK,		false,	fAtk * 1.8f };
	m_Attack5_Desc	=	{ wstrOwnerName, L"Player_Zhongli Attack5", NONE_ELEMENTAL, STRONG,		false,	fAtk * 1.0f };
										   
	m_Skill1_Desc	=	{ wstrOwnerName, L"Player_Zhongli Skill1", GEO, WEAK,		false,	fAtk * 5.0f };
	m_Skill2_Desc	=	{ wstrOwnerName, L"Player_Zhongli Skill2", GEO, POWERFUL,	false,	fAtk * 10.0f };
...
////////////////
	...
	/* ATTACK */
	m_fAttack_ExitTime = 0.3f;	// 공격 도중 이 시간이 지나야 탈출 가능
	...
	/* ELEMENTAL_BURST */
	m_fElemental_Burst_MinTime = 2.5f;	// 스킬 사용후 다음 행동을 하기까지 필요한 최소 시간
...
_int CZhongli_StateMachine::PreUpdate(const _float& _fTimeDelta)
{
	return CStateMachine::PreUpdate(_fTimeDelta);
}

_int CZhongli_StateMachine::Update(const _float& _fTimeDelta)
{
...
_int CZhongli_StateMachine::LateUpdate(const _float& _fTimeDelta)
{
	return CStateMachine::LateUpdate(_fTimeDelta);
}

void CZhongli_StateMachine::Render()
{
	CStateMachine::Render();
}

void CZhongli_StateMachine::EndPlay()
{
	CStateMachine::EndPlay();
}

void CZhongli_StateMachine::Release()
{
...
void CZhongli_StateMachine::ManagementActorState(const _float& _fTimeDelta)
{
...
#pragma region [ NORMAL ATTACK ]
		if (m_iCurrState & IS_NORMALATTACK)
		{
			if (m_iCurrState & PLAYER_STATE::ATTACK_1)
			{
				...
				NormalAttackColliderOnOff(0.2f, 0.3f);
			}
			...
			else if (m_iCurrState & PLAYER_STATE::ATTACK_5)
			{
				...
				if (0.2f < m_fAttack_RunTime && !m_bAttack5_One_Completed)
				{
					m_Attack5_Desc.wstrAttackName = L"Player_Zhongli Attack5" + to_wstring(rand());
					m_wpWeaponCollider.lock()->SetAttackDesc(m_Attack5_Desc);
					m_bAttack5_One_Completed = true;
				}
				else if (0.3f < m_fAttack_RunTime && !m_bAttack5_Two_Completed)
				{
					m_Attack5_Desc.wstrAttackName = L"Player_Zhongli Attack5" + to_wstring(rand());
					m_wpWeaponCollider.lock()->SetAttackDesc(m_Attack5_Desc);
					m_bAttack5_Two_Completed = true;
				}
				else if (0.4f < m_fAttack_RunTime && !m_bAttack5_One_Completed)
				{
					m_Attack5_Desc.wstrAttackName = L"Player_Zhongli Attack5" + to_wstring(rand());
					m_wpWeaponCollider.lock()->SetAttackDesc(m_Attack5_Desc);
					m_bAttack5_Three_Completed = true;
				}
				else if (0.5f < m_fAttack_RunTime && !m_bAttack5_Two_Completed)
				{
					m_Attack5_Desc.wstrAttackName = L"Player_Zhongli Attack5" + to_wstring(rand());
					m_wpWeaponCollider.lock()->SetAttackDesc(m_Attack5_Desc);
					m_bAttack5_Four_Completed = true;

				}
				else if (0.6f < m_fAttack_RunTime && !m_bAttack5_Two_Completed)
				{
					m_Attack5_Desc.wstrAttackName = L"Player_Zhongli Attack5" + to_wstring(rand());
					m_wpWeaponCollider.lock()->SetAttackDesc(m_Attack5_Desc);
					m_bAttack5_Five_Completed = true;
				}

				if (0.4f < m_fAttack_RunTime && !m_bAttack5_SoundCompleted)
				{
					m_bAttack5_SoundCompleted = true;
					PlayHeavyAttackVoice();
				}

				NormalAttackColliderOnOff(0.2f, 7.f);
			}
		}
...
#pragma region [ SKILL ]
		else if (m_iCurrState & IS_SKILL)
		{
			...
			{
				if (0.85f <= m_fElemental_Skill_RunTime && m_fElemental_Skill_RunTime < 1.f)
					m_wpSkillCollider.lock()->SetIsCollideOn(true);
				else
					m_wpSkillCollider.lock()->SetIsCollideOn(false);

				if (0.7f < m_fElemental_Skill_RunTime && !m_bElementalSkillSoundCompleted)
				{
					m_bElementalSkillSoundCompleted = true;
	...
	else
	{
		...
		m_bAttack5_Three_Completed = false;
		m_bAttack5_Four_Completed = false;
		m_bAttack5_Five_Completed = false;

		m_bAttack5_SoundCompleted = false;
	}
#pragma endregion [ SKILL ]

}

void CZhongli_StateMachine::ManagementActorCoolTime(const _float& _fTimeDelta)
{
	/* == 이동 == */
...
void CZhongli_StateMachine::RequestChangeState(_ullong _iPlayerStateFlag, REQUEST_DIR _eDirection, ATTACK_DESC _AttackDesc)
{
...
#pragma region [ FLYING ]
		if (_iPlayerStateFlag & IS_FLYING)
		{
			if (_iPlayerStateFlag & PLAYER_STATE::FLY_START)
			{
				...
				CSoundManager::GetInstance()->PlaySoundEx("Zhongli_FlyStart.ogg", SOUND_TYPE::VOICE, 0.1f);
			}
...
#pragma region [ NORMAL ATTACK]
		if (_iPlayerStateFlag & IS_NORMALATTACK && m_fAttack_ExitTime <= m_fAttack_RunTime)
		{
			...
			m_bAttack5_SoundCompleted = false;
			m_bElementalSkillSoundCompleted = false;
...
#pragma region [ SKILL ]
		else if ((_iPlayerStateFlag & PLAYER_STATE::ELEMENTAL_CLICK || _iPlayerStateFlag & PLAYER_STATE::ELEMENTAL_CHARGE) && m_fElemental_Skill_CoolTime <= m_fElemental_Skill_RunTime)
		{
			if (_iPlayerStateFlag & PLAYER_STATE::ELEMENTAL_CLICK)
			{
				...
				MakeShield(L"Zhongli_Skill_E_ShieldCylinder");
				MakeShield(L"Zhongli_Skill_E_ShieldRing_One");
				MakeShield(L"Zhongli_Skill_E_ShieldRing_Two");
				MakeEffect(L"Zhongli_Skill_E_ShockWave", XMVectorSet(0.f, 5.f, 0.f, 0.f));
				MakeEffect(L"Zhongli_Skill_E_Floor", XMVectorSet(0.f, 5.f, 0.f, 0.f));

				CSoundManager::GetInstance()->PlaySoundEx("Zhongli_ElementalSkill.mp3", EFFECT_7, 0.5f);
			}
		...
		else if ((_iPlayerStateFlag & PLAYER_STATE::ELEMENTAL_BURST) && m_fElemental_Burst_CoolTime <= m_fElemental_Burst_RunTime)
		{
			...
			MakeJusin(m_Skill2_Desc, L"Zhongli_Skill_Q_Jusin");
...
#pragma region 연출
			 ...
			 _vector vStartCamPos = vPlayerPos + (vPlayerRight * 50.f) + (vPlayerUp * 100.f) + (vPlayerLook * 50.f);
			 _vector vEndCamPos = vPlayerPos + (vPlayerRight * 60.f) + (vPlayerUp * 100.f) + (vPlayerLook * 10.f);
			 
			 _vector vStartCamAt = vPlayerPos + (vPlayerUp * 150.f);
			 _vector vEndCamAt = vPlayerPos + (vPlayerUp * 150.f);
			 
			 spProductionCam->BeginProduction(vStartCamPos, vEndCamPos, vStartCamAt, vEndCamAt, 1.5f);
			 spProductionCam->SetProductionRatio(1.f, 0.6f, 0.6f);
#pragma endregion 연출

			 PlayElementalBurstVoice();
			 CSoundManager::GetInstance()->PlaySoundEx("Zhongli_ElementalBurst.mp3", EFFECT_7, 0.5f);

			 m_Skill2_Desc.wstrAttackName = L"Player_Zhongli Skill2" + to_wstring(rand());
...
void CZhongli_StateMachine::MakeJusin(ATTACK_DESC& _AttackDesc, const wstring& _wstrEffectName)
{
	...
	shared_ptr<CJusin> spJusin;

	if (nullptr != spActor)
		spJusin = dynamic_pointer_cast<CJusin>(spActor);
	...
	if (nullptr != spJusin)
	{
		...
		spJusin->SetActorTranslation_NoNavigation(vOwnerPos + XMVectorSet(0.f, 1500.f, 0.f, 0.f) - vOwnerLook * 1000.f);
		spJusin->SetShootDirrection(XMVector3Normalize(vOwnerLook) + XMVectorSet(0.f, -1.f, 0.f, 0.f));
		spJusin->SetShootSpeed(1200.f);
		spJusin->SetExplosionPos(vOwnerPos);
		spJusin->SetAttackDesc(_AttackDesc);
	}
}

void CZhongli_StateMachine::MakeShield(const wstring& _wstrEffectName)
{
	shared_ptr<CActor> spActor = GetLevel().lock()->GetLayer(LAYER::LAYER_EFFECTMESH)->RecycleFromPool(_wstrEffectName);
	...
	shared_ptr<CShield> spShield = dynamic_pointer_cast<CShield>(spActor);

	spShield->SetOwnerActor(m_wpOwnerActor);
}

void CZhongli_StateMachine::MakeParticle_E(_vector _vOffset)
{
...
void CZhongli_StateMachine::MakeParticle_Q(_vector _vOffset)
{
...
void CZhongli_StateMachine::PlayNormalAttackVoice()
{
	...
	if (0 == m_iSoundCount_LightAttack % 2)
	{
		...
		case 0:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_NormalAttack1.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 1:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_NormalAttack2.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 2:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_NormalAttack3.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 3:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_NormalAttack4.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 4:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_NormalAttack5.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 5:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_NormalAttack6.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 6:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_NormalAttack7.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		}
	}
}
void CZhongli_StateMachine::PlayHeavyAttackVoice()
{
	...
	case 0:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyAttack1.ogg", SOUND_TYPE::VOICE, 0.3f);
		break;
	}
	case 1:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyAttack2.ogg", SOUND_TYPE::VOICE, 0.3f);
		break;
	}
	case 2:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyAttack3.ogg", SOUND_TYPE::VOICE, 0.3f);
		break;
	}
	}
}
void CZhongli_StateMachine::PlayHitVoice(ATTACK_DESC _AttackDesc)
{
	// 4. 피격 사운드 재생
	...
	case KNOCKBACK_TYPE::STRONG:
	{
		...
		case 0:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Hit_L1.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 1:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Hit_L2.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 2:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Hit_L3.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 3:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Hit_L4.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 4:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Hit_L5.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 5:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Hit_L6.ogg", SOUND_TYPE::VOICE, 0.5f);
			break;
		}
		}
		break;
	...
	case KNOCKBACK_TYPE::POWERFUL:
	{
		...
		case 0:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyHit1.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 1:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyHit2.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 2:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyHit3.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 3:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyHit4.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 4:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyHit5.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		case 5:
		{
			CSoundManager::GetInstance()->PlaySoundEx("Zhongli_HeavyHit6.ogg", SOUND_TYPE::VOICE, 0.3f);
			break;
		}
		}
		break;
...
void CZhongli_StateMachine::PlaySprintVoice()
{
	...
	case 0:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_SprintStart1.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	case 1:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_SprintStart2.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	case 2:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_SprintStart3.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	}
}
void CZhongli_StateMachine::PlayJumpVoice()
{
	++m_iSoundCount_Jump;
	...
	case 0:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Jump1.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	case 1:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Jump2.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	case 2:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Jump3.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	case 3:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Jump4.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	case 4:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Jump5.ogg", SOUND_TYPE::VOICE, 0.5f);
		break;
	}
	}
}

void CZhongli_StateMachine::PlayElementalSkillVoice()
{
	...
	case 0:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Elemental_Skill1.ogg", SOUND_TYPE::VOICE, 0.3f);
		break;
	}
	case 1:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Elemental_Skill2.ogg", SOUND_TYPE::VOICE, 0.3f);
		break;
	}
	case 2:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Elemental_Skill3.ogg", SOUND_TYPE::VOICE, 0.3f);
		break;
	}
	}
}
void CZhongli_StateMachine::PlayElementalChargeVoice()
{
}
void CZhongli_StateMachine::PlayElementalBurstVoice()
{
	...
	case 0:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Elemental_Burst1.ogg", SOUND_TYPE::VOICE, 0.6f);
		break;
	}
	case 1:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Elemental_Burst2.ogg", SOUND_TYPE::VOICE, 0.6f);
		break;
	}
	case 2:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Zhongli_Elemental_Burst3.ogg", SOUND_TYPE::VOICE, 0.6f);
		break;
	}
	}
}

void CZhongli_StateMachine::PlayRunSound_FootSteps()
{
	switch (m_iRunSoundCount % 4)
	{
	case 0:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Banks11_662.ogg", SOUND_TYPE::EFFECT_2, 0.3f);
		break;
	}
	case 1:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Banks11_663.ogg", SOUND_TYPE::EFFECT_2, 0.3f);
		break;
	}
	case 2:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Banks11_664.ogg", SOUND_TYPE::EFFECT_2, 0.3f);
		break;
	}
	case 3:
	{
		CSoundManager::GetInstance()->PlaySoundEx("Banks11_665.ogg", SOUND_TYPE::EFFECT_2, 0.3f);
		break;
	}
	}
...
void CZhongli_StateMachine::PlaySprintSound_FootSteps()
{
}