#include "pch.h"
#include "Slime_Fire_StateMachine.h"
...
void CSlime_Fire_StateMachine::Initialize()
{
	...
	m_Attack1_Desc = { wstrOwnerName, L"Slime_Fire Attack1", PYRO, WEAK,	false, fAtk * 1.0f };
	m_Attack2_Desc = { wstrOwnerName, L"Slime_Fire Attack2", PYRO, STRONG,	false, fAtk * 2.0f };
	m_Attack3_Desc = { wstrOwnerName, L"Slime_Fire Attack3", PYRO, WEAK,	false, fAtk * 2.0f };
	m_Attack4_Desc = { wstrOwnerName, L"Slime_Fire Attack4", PYRO, WEAK,	false, fAtk * 1.0f };
...
_int CSlime_Fire_StateMachine::PreUpdate(const _float& _fTimeDelta)
{
	return CMonsterStateMachine::PreUpdate(_fTimeDelta);
}

_int CSlime_Fire_StateMachine::Update(const _float& _fTimeDelta)
{
...
_int CSlime_Fire_StateMachine::LateUpdate(const _float& _fTimeDelta)
{
	return CMonsterStateMachine::LateUpdate(_fTimeDelta);
}

void CSlime_Fire_StateMachine::Render()
{
	CMonsterStateMachine::Render();
}

void CSlime_Fire_StateMachine::EndPlay()
{
	CMonsterStateMachine::EndPlay();
}

void CSlime_Fire_StateMachine::Release()
{
	CMonsterStateMachine::Release();
}

void CSlime_Fire_StateMachine::ManagementActorState(const _float& _fTimeDelta)
{
...
void CSlime_Fire_StateMachine::ManagementActorCoolTime(const _float& _fTimeDelta)
{
	/* Attack1 */
...
void CSlime_Fire_StateMachine::RequestChangeState(_ullong _iMonsterStateFlag, REQUEST_DIR _eDirection, ATTACK_DESC _AttackDesc)
{
	...
	{
		m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::DIE1);
	...
	else if (_iMonsterStateFlag & IS_HIT)
	{
		...
		case MONSTER_STATE::HIT1:
			m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::HIT_L);
			m_tInfo.m_fHit1RunTime = 0.0f;
			break;

		case MONSTER_STATE::HIT2:
			m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::HIT_H);
			m_tInfo.m_fHit2RunTime = 0.0f;
			break;

		case MONSTER_STATE::HIT3:
			m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::HIT_THROW_TO_GROUND);
	...
	else if (_iMonsterStateFlag & IS_MOVE)
	{
		...
		if (_iMonsterStateFlag & MONSTER_STATE::PROWL)
			m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::WALK_FORWARD);
		else if (_iMonsterStateFlag & MONSTER_STATE::TRACE)
			m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::WALK_FORWARD);
		else if (_iMonsterStateFlag & MONSTER_STATE::DETECTED)
			m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::BORN);
	...
	else if (_iMonsterStateFlag & IS_ATTACK)
	{
		...
		if (_iMonsterStateFlag & IS_NORMALATTACK)
		{
			if (_iMonsterStateFlag & IS_ATTACK_START)
			{
				...
				{
				case ATTACK1_START:
					m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::NORMALATTACK);
				...
				case ATTACK2_START:
					m_wpAnimModel.lock()->SetAnimationIndex(SLIME_FIRE_ANIM::COMPRESS_ATTACK);
