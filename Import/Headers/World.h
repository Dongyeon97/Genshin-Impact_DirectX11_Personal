...
class ENGINE_DLL CWorld : public std::enable_shared_from_this<CWorld>
{
public:
...
public:
	...
	virtual _int Update(const float& _fDeltaSeconds);
	virtual _int LateUpdate(const float& _fDeltaSeconds);
...
private:
	 ...
	 * @return std::shared_ptr<T> 새로 생성한 레벨의 셰어드 포인터를 반환합니다. ※문자열이 중복되면 안됩니다.※
	 * @param[in] _wstrLevel 레벨 문자열
	 */
	template<typename T>
...
public:
	...
	@brief 현재 레벨을 불러옵니다.
	*/
	std::weak_ptr<CLevel> GetLevel() { assert(!m_wpLevel.expired());  return m_wpLevel; }
